...
class KPIEvaluationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        # YENİ ƏLAVƏ (Tələb 1): Top management dəyərləndirilə bilməz
        if evaluatee.role == 'top_management':
            raise PermissionDenied("'Top Management' rolundakı istifadəçilər dəyərləndirilə bilməz.")
        ...
        if evaluator == evaluatee:
            ...
            # YENİ ƏLAVƏ (Tələb 2): Yalnız birbaşa rəhbər dəyərləndirə bilsin
            # Admin bu qaydanı istisna edir (is_staff yoxlaması ilə)
            direct_superior = evaluatee.get_superior()
            if evaluator != direct_superior and not evaluator.is_staff:
                raise PermissionDenied("Bu işçini yalnız onun birbaşa rəhbəri dəyərləndirə bilər.")

            if not evaluator.is_staff: # Admin istisna olmaqla
    ...
    def kpi_dashboard_tasks(self, request):
        ...
        # YENİ ƏLAVƏ (Tələb 1): Top managementə təyin edilmiş tapşırıqları xaric et
        all_tasks = all_tasks.exclude(assignee__role='top_management')
