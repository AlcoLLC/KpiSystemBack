...
class DailyProduction(models.Model):
    ...
    employees = models.ManyToManyField(settings.AUTH_USER_MODEL, related_name='productions', verbose_name="İşçilər")
    ...
    def calculate_results(self):
        ...
        weighted_sum = sum((item.efficiency * item.production_hours) for item in items)
        self.final_efficiency = round(weighted_sum / 8, 2)
        self.save()
    ...
    def employee_details(self):
        return [{"id": emp.id, "full_name": f"{emp.first_name} {emp.last_name}"} for emp in self.employees.all()]

class ProductionItem(models.Model):
    production = models.ForeignKey(DailyProduction, related_name='items', on_delete=models.CASCADE)
    volume = models.ForeignKey(EquipmentVolume, on_delete=models.CASCADE, verbose_name="Tiraj")
